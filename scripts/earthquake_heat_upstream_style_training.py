#!/usr/bin/env python3
"""Train Earthquake Heat with Upstream's sigma-scaled score parameterization.

The teacher, linear beta schedule, physical-time sampling, and existing reverse
sampler are provided by ``experiment_earthquake_teacher_compare_smoke``.  This
entry point fixes only the new independent score-training path and supplies the
100k-EMA Heat experiment defaults; explicit CLI arguments may override the
non-parameterization defaults.
"""
...
DEFAULT_ARGUMENTS = (
    ("--output-dir", "results/earthquake_heat_upstream_style_training"),
    ("--train-size", "4096"),
...
def configured_arguments(arguments: list[str]) -> list[str]:
    ...
    if _has_option(configured, "--teacher"):
        raise ValueError("this experiment fixes --teacher heat")
